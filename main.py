...
def find_neighours_closer_to_target(current_town, target_town, number_of_neighbours, towns_dict):
    ...
    for i in range(0, len(current_neighbours)):
        if distance.distance(current_neighbours[i], target_town) < distance_without_deviation:
            ...
            print(
                f"{get_name_of_town(current_neighbours[i], towns_dict)} is closer to {get_name_of_town(target_town, towns_dict)} than {get_name_of_town(current_town, towns_dict)}")
...
def get_name_of_town(town_coords, towns_dict):
    for key in towns_dict.keys():
        if towns_dict[key] == town_coords:
            return key


def dijkstra_algorithm(origin_town, target_town, towns_dict):
    my_root_of_tree = Node(origin_town)
    print(f"Root: {my_root_of_tree}")
    print(f"Name of town: {get_name_of_town(my_root_of_tree.name, towns_dict)}")
